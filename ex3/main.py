...
from data_handler import load_data, transform_func, extract_features
...
def fit_vae(num_labels, is_save_tensorboard, params):
    ...
    # Set Data
    latent_train_loader, classifier_train_loader, test_loader = load_data(batch_size, num_labels=100, transform_func=transform_func)
...
def main():
    ...
    for _labels in LABELS:
        ...
        model_utils.save_model(model=vae_models[_labels],
                               output_path=os.path.join(MODELS_OUTPUT_DIR, f"vae_{_labels}_labels_datatype_{DATA_TYPE}_model.pth"))
        ...
        model_utils.save_model(model=svm_models[_labels],
                               output_path=os.path.join(MODELS_OUTPUT_DIR, f"svm_{_labels}_labels_datatype_{DATA_TYPE}_model.pth"))
